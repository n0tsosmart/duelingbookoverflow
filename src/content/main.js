...
function injectFunction(extensionId, unlockCardMechanics, lowAnimations, silentCommands, birdUI, psctColors, cardLogging, goatCards, edisonCards, negate_icon_blob, skipIntro, autoConnect, extensionBaseUrl) {
	...
	let DBO_offlineDeckViewer = $("#front_page #DBO_offlineDeckViewer")

	if (DBO_offlineDeckViewer.length == 0) {
		DBO_offlineDeckViewer = $(`<div></div>`)
		DBO_offlineDeckViewer.setHTML(`<div class="menu_btn" style="top: 550px; left: 550px; opacity: 1; cursor: pointer;">
		...
		DBO_offlineDeckViewer.attr("id", "DBO_offlineDeckViewer");

		// Create new clickable button outside of the screen
		DBO_offlineDeckViewer.append($(`<div style="top: 5000px"><button></button></div>`))

		// Add the whole button to the front page
		$('#front_page').append(DBO_offlineDeckViewer)
	}

	if (DBO_offlineDeckViewer.length > 0 && typeof gotoDeckConstructor !== 'undefined') {
		DBO_offlineDeckViewer.off("click");
		DBO_offlineDeckViewer.on("click", gotoDeckConstructor);
		DBO_offlineDeckViewer.on("click", searchCardsE);
		DBO_offlineDeckViewer.on("click", function () { $('#welcome_buttons').hide() });

		DBO_offlineDeckViewer.off("touchend");
		DBO_offlineDeckViewer.on("touchend", gotoDeckConstructor);
		DBO_offlineDeckViewer.on("touchend", searchCardsE);
		DBO_offlineDeckViewer.on("touchend", function () { $('#welcome_buttons').hide() });
	}
